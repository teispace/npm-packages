...
describe('README numbers match reality', () => {
  ...
  it('quotes the real conformance fixture counts', () => {
    // Both figures appear twice in the README, once for the encoder and once
    // for the decoder, and both must match the files on disk — the fixture
    // sets are regenerated by scripts, so a stale number is easy to leave.
    const read = (file: string): number =>
      JSON.parse(readFileSync(join(import.meta.dirname, 'fixtures', file), 'utf8')).cases.length;

    const micro = [...README.matchAll(/(\d+) fixtures, checked on every test run/g)];
    expect(micro).toHaveLength(1);
    expect(Number(micro[0][1])).toBe(read('micro-qr.json'));
    expect(README).toContain(`reads all ${read('micro-qr.json')} of those`);

    const rmqr = [...README.matchAll(/(\d+) fixtures, each pinning its/g)];
